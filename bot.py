...
class Handlers:
    ...
    def _register_common(self):
        ...
        async def cmd_ban(message: types.Message):
            ...
            try:
                await self.bot.send_message(target_id, "⛔ Вы забанены администратором.")
            except:
                pass
        ...
        async def handle_menu_buttons(message: types.Message):
            ...
            if not user or not user['accepted_rules']:
                await message.reply("Используйте /start для начала.")
                return
            ...
            if user['is_blocked'] and message.text != "📝 Проверить комментарий":
                ...
                await message.reply(
                    f"⛔ Доступ заблокирован. Нужно {Config.COMMENT_THRESHOLD} комментариев.\n"
    ...
    def _register_admin(self):
        ...
        # Обработчики состояний (копируем из оригинального бота)
        @self.dp.message_handler(lambda m:
    ...
    async def _send_main_menu(self, chat_id: int, user_id: int):
        ...
        if blocked:
            ...
            text = (
                ...
                f"Отправляйте скриншоты с @{Config.BOT_NAME}!"
            )
    ...
    async def _show_rules(self, chat_id: int):
        ...
        text = (
            ...
            f"📱 *Что умеет бот:*\n"
            f"• Проверка комментариев с @{Config.BOT_NAME}\n"
            f"• Накопление комментариев для доступа\n"
            f"• Выполнение заданий\n"
            f"• Вывод средств\n\n"
            f"💰 *Примерные заработки:* 500-1500₽ в неделю\n\n"
            f"📊 *Система:*\n"
            f"• Для разблокировки нужно {Config.COMMENT_THRESHOLD} комментариев\n"
            f"• Каждый понедельник списывается {Config.WEEKLY_COMMENT_DECREMENT}\n"
            f"• Если баланс 0 - доступ блокируется\n\n"
            f"💳 *Вывод:*\n"
    ...
    async def _send_help(self, message: types.Message):
        help_text = (
            f"❓ *Помощь:*\n\n"
            f"📝 *Проверить комментарий* — отправьте скриншот с @{Config.BOT_NAME}, получите +1.\n"
            f"💰 *Мой баланс* — балансы и статус.\n"
            f"💎 *Вывод средств* — заявка на вывод.\n"
            f"📊 *Статистика* — ваша статистика.\n\n"
            f"🔒 *Система:*\n"
            f"• {Config.COMMENT_THRESHOLD} комментов для разблокировки\n"
            f"• Каждый понедельник -{Config.WEEKLY_COMMENT_DECREMENT}\n"
            f"• За обман - бан навсегда"
        )
    ...
    async def _handle_check_comment(self, message: types.Message):
        ...
        if now - last < Config.ANTIFLOOD_SECONDS:
            ...
            await message.reply(f"⏳ Подождите {remaining} секунд.")
            return

        await self.state_manager.set_state(user_id, UserState.WAITING_PHOTO)

        await message.reply(
            f"📸 Отправьте скриншот с упоминанием @{Config.BOT_NAME}.\n\n"
            f"⚠️ За любое фото сразу +1 комментарий.\n"
            f"Если админ заметит обман - бан навсегда.\n\n"
    ...
    async def _handle_photo(self, message: types.Message):
        ...
        if not await self.state_manager.has_state(user_id, UserState.WAITING_PHOTO):
            await message.reply("❌ Сначала нажмите '📝 Проверить комментарий'.")
            return
        ...
        if not message.photo:
            await message.reply("❌ Ошибка: фото не обнаружено.")
            return

        photo = message.photo[-1]
        if photo.file_size > Config.MAX_PHOTO_SIZE:
            await message.reply(f"❌ Файл слишком большой. Максимум {Config.MAX_PHOTO_SIZE_MB} MB.")
            return
        ...
        if await self.db.check_photo_hash(photo_hash):
            await message.reply("❌ Этот скриншот уже использовался.")
            return

        # Сохраняем хэш
        await self.db.save_photo_hash(user_id, photo_hash)

        # НАЧИСЛЯЕМ КОММЕНТАРИЙ СРАЗУ
        new_balance = await self.db.add_comment(user_id)

        # Получаем данные пользователя для лога
        user = await self.db.get_user(user_id)
        username = user.get('username') or f"{user['first_name']} {user['last_name']}".strip() or "NoName"
        ...
        # Отправляем фото админу с логом (БЕЗ КНОПОК)
    ...
    # ===== МЕТОДЫ ДЛЯ ВЫВОДА, РАССЫЛКИ, УПРАВЛЕНИЯ =====
    ...
    async def _handle_withdraw_details(self, message: types.Message):
        ...
        if method == 'card':
            ...
            if len(card) != 16:
                await message.reply("Некорректный номер карты. Введите 16 цифр.")
                return
            details = card
    ...
    async def _handle_broadcast_link(self, message: types.Message):
        ...
        await message.reply("Введите сумму награды за выполнение задания (рублей):")
    ...
    async def _callback_complete_task(self, call: types.CallbackQuery):
        ...
        await call.message.reply(f"✅ Спасибо! Начислено {reward}₽ на ваш баланс.")
        
        if link:
            await call.message.reply(f"🔗 Ваша ссылка: {link}")
    ...
    async def _show_admin_stats(self, message: types.Message):
        ...
        banned_count = await self.db._execute(
        ...
        text = (
            ...
            f"✅ Активных: {active}\n"
            f"⛔ Забанено навсегда: {banned_count}\n"
    ...
    async def _callback_withdrawal_action(self, call: types.CallbackQuery):
        ...
        if action == 'approve':
            ...
            try:
                await self.bot.send_message(
                    w['user_id'],
                    f"✅ Ваша заявка на вывод {w['amount']}₽ принята."
                )
            except:
                pass
