...
def handle_admin_buttons(message: types.Message):
    ...
    if text == "👥 Рассылка":
        ...
        start_balance_management(message)
    elif text == "📊 Статистика":
        show_admin_stats(message)
    elif text == "📤 Экспорт ID":
        export_user_ids(message)
    elif text == "🔧 Тикеты на выплату":
        show_pending_withdrawals(message)
...
def start_broadcast(message: types.Message):
    ...
    bot.send_message(message.chat.id, "Выбери тип аудитории:", reply_markup=markup)


@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_target_type'))
def handle_broadcast_target_type(message: types.Message):
    ...
    if text == "1️⃣ Все пользователи":
        ...
        bot.send_message(message.chat.id, "Введите количество пользователей для выборки:")
    else:
        bot.send_message(message.chat.id, "Пожалуйста, выберите пункт меню.")


@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_count'))
def handle_broadcast_count(message: types.Message):
...
@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_sort'))
def handle_broadcast_sort(message: types.Message):
...
@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_text'))
...
@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_link'))
def handle_broadcast_link(message: types.Message):
...
@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'broadcast_reward'))
def handle_broadcast_reward(message: types.Message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Введите целое неотрицательное число.")
        return
    ...
    if data['target_type'] == 'all':
        ...
        user_ids = db.get_users_for_broadcast(data['target_type'], data['count'])
    ...
    if data.get('link'):
        ...
        # Используем callback с ID рассылки для начисления награды
        callback_data = f"complete_{broadcast_db_id}_{reward}"
        markup.add(types.InlineKeyboardButton("✅ Выполнить", callback_data=callback_data))
    ...
    for uid in user_ids:
        try:
            bot.send_message(uid, data['message_text'], reply_markup=markup)
            sent += 1
...
def callback_complete_task(call: types.CallbackQuery):
    ...
    # (можно добавить проверку в БД, но для простоты пропустим)
    ...
    bot.answer_callback_query(call.id, "Задание выполнено! Награда начислена.")
...
def start_balance_management(message: types.Message):
...
@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'manage_balances_search'))
def handle_balance_search(message: types.Message):
    ...
    markup.add(
        types.InlineKeyboardButton("➕ Начислить комментарии", callback_data="mod_comment_add"),
        types.InlineKeyboardButton("➖ Списать комментарии", callback_data="mod_comment_sub"),
        types.InlineKeyboardButton("➕ Начислить деньги", callback_data="mod_money_add"),
        types.InlineKeyboardButton("➖ Списать деньги", callback_data="mod_money_sub"),
        types.InlineKeyboardButton("🔄 Заблокировать/Разблокировать", callback_data="mod_toggle_block"),
        types.InlineKeyboardButton("🔙 Завершить", callback_data="mod_finish")
    )
...
def callback_balance_modification(call: types.CallbackQuery):
    ...
    if data == 'mod_comment_add':
        ...
        bot.send_message(call.message.chat.id, f"✅ Статус блокировки изменен.")
        # Возвращаемся к меню
        start_balance_management(call.message)
    elif data == 'mod_finish':
        user_state.clear_state(admin_id)
        bot.answer_callback_query(call.id, "Готово.")
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        send_main_menu(call.message.chat.id, admin_id, bot, db)


@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, [
    'manage_balances_comment_add', 'manage_balances_comment_sub',
    'manage_balances_money_add', 'manage_balances_money_sub'
]))
def handle_balance_change(message: types.Message):
...
def show_admin_stats(message: types.Message):
...
def export_user_ids(message: types.Message):
...
def show_pending_withdrawals(message: types.Message):
    ...
    for w in withdrawals:
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("✅ Принять", callback_data=f"approve_{w['id']}"),
            types.InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{w['id']}")
        )
...
def callback_withdrawal_action(call: types.CallbackQuery):
    ...
    if action == 'approve':
        ...
        bot.send_message(admin_id, "Напишите причину отказа:")


@bot.message_handler(func=lambda message: user_state.has_state(message.from_user.id, 'waiting_reject_reason'))
def handle_reject_reason(message: types.Message):
    ...
    try:
        bot.delete_message(data['msg'].chat.id, data['msg'].message_id)
    except:
        pass

    user_state.clear_state(admin_id)

    bot.send_message(admin_id, "✅ Заявка отклонена.")